...
- **mcpproxy** – `home/private_dot_mcpproxy/modify_mcp_config.json.tmpl` (→ `~/.mcpproxy/mcp_config.json`). Renders the [smart-mcp-proxy/mcpproxy-go](https://github.com/smart-mcp-proxy/mcpproxy-go) config from `home/.chezmoidata/mcps/*.yaml` (same source of truth as Cursor and OpenCode). Installed via Homebrew tap `smart-mcp-proxy/mcpproxy` on macOS/Linux; not deployed on Windows. Use the `update-mcp-servers` skill when changing servers.

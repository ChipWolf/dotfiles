...
function chatCompletionsNonStream(res) {
	json(res, 200, {
		id: "chatcmpl_audit",
...
const server = createServer((req, res) => {
	...
	req.on("end", () => {
		...
		if (
			req.method === "GET" &&
			(url.includes("/v1/models") || url.endsWith("/models"))
		) {
			json(res, 200, {
				object: "list",
				data: [{ id: "audit-model", object: "model" }],
			});
			return;
		}
		...
		if (
			...
			const isTitleRequest =
				/title generator|Generate a title for this conversation/i.test(body);
			if (
				!captured &&
				!(process.env.SKIP_TITLE_REQUEST === "1" && isTitleRequest)
			) {
				captured = true;
				...
				if (url.includes("/responses")) {
					...
					let parsedStream;
					try {
						parsedStream = JSON.parse(body).stream;
					} catch {
						parsedStream = undefined;
					}
					if (parsedStream === false || parsedStream === undefined) {
						chatCompletionsNonStream(res);
					} else {
			...
			if (url.includes("/responses")) {
				...
				let parsedStream2;
				try {
					parsedStream2 = JSON.parse(body).stream;
				} catch {
					parsedStream2 = undefined;
				}
				if (parsedStream2 === false || parsedStream2 === undefined) {
					chatCompletionsNonStream(res);
				} else {
					chatCompletionsStream(res);
				}

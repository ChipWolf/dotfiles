...
- **Performance recommendations need variance, not a single sample.** When recommending a change based on benchmark or timing data, gather multiple samples (n≥3, ideally n≥5 for cloud or CI workloads), characterise variance, and present it. A single run is exploratory data, not a basis for a production PR or design decision. Small CI runner labels and any managed-CI workload tend to be noisy, and platforms like Blacksmith oversubscribe smaller-labeled runners onto bigger physical hosts, so the labeled size is not necessarily what got measured. Pin worker counts (e.g. `PARALLEL_PROCESS_NUMBER`, `--threads=N`) so the workload uses the size you asked for, not the host you happened to land on.
- **Servers re-serialize their configs on boot.** When writing a regression test that asserts on a config file the server can rewrite (Minecraft plugin YAMLs, BlueMap HOCON, Paper config, etc.), strict greps like `^foo: bar$` will fail post-boot because the server reformats the file (added quotes, whitespace tweaks, key reordering, env-var substitution). Use flexible regex (`[[:space:]]*foo[[:space:]]*[:=][[:space:]]*"?bar"?`), anti-default checks (`! grep upstream-default`) for env-substituted values, or move the assertion to a static lint over the committed file when post-boot state is unreliable. Verify the post-boot file format with at least one CI run before pinning the assertion.
